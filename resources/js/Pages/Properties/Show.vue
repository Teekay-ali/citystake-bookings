...
const selectedImage = ref(props.property.images[0]?.image_path || null);
const checkIn = ref('');
const checkOut = ref('');
const guests = ref(2);
...
const estimatedTotal = computed(() => {
    ...
    const subtotal = calculateNights.value * props.property.base_price_per_night;
    const serviceCharge = subtotal * (props.property.service_charge_percent / 100);
    return subtotal + props.property.cleaning_fee + serviceCharge;
});
...
const checkAvailability = async () => {
    ...
    try {
        const response = await fetch(route('properties.check-availability', props.property.slug), {
            method: 'POST',
            headers: {
...
const proceedToBooking = () => {
    ...
    router.get(route('bookings.create', props.property.slug), {
        check_in: checkIn.value,
        check_out: checkOut.value,
        guests: guests.value,
    });
        ...
        <div class="bg-white dark:bg-gray-950 min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-6 lg:px-8 py-8">
                <div class="grid grid-cols-4 gap-2 rounded-3xl overflow-hidden h-[600px]">
                    <!-- Main Image -->
                    <div class="col-span-4 md:col-span-2 md:row-span-2 relative group cursor-pointer" @click="selectedImage = property.images[0]?.image_path">
                        <img
                    ...
                    <template v-if="property.images.length > 1">
                        ...
                        >
                            ...
                            <div v-if="index === 3 && property.images.length > 5" class="absolute inset-0 bg-black/60 flex items-center justify-center">
                                <span class="text-white text-sm font-medium">+{{ property.images.length - 5 }} more</span>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="max-w-7xl mx-auto px-6 lg:px-8 pb-24">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
                    ...
                    <div class="lg:col-span-2 space-y-12">
                        ...
                        <!-- House Rules -->
                        <div v-if="property.house_rules && property.house_rules.length">
                            ...
                            <div class="border border-gray-200 dark:border-gray-800 rounded-3xl p-8 bg-white dark:bg-gray-900 shadow-sm">
                                ...
                                <div v-if="calculateNights > 0" class="space-y-3 mb-6 pb-6 border-b border-gray-100 dark:border-gray-800">
                                    <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                                        <span>{{ formatPrice(property.base_price_per_night) }} × {{ calculateNights }} nights</span>
                                        <span>{{ formatPrice(calculateNights * property.base_price_per_night) }}</span>
                                    </div>
                                    <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                                        ...
                                        <span>{{ formatPrice((calculateNights * property.base_price_per_night) * (property.service_charge_percent / 100)) }}</span>
                                    </div>
                                </div>
                                ...
                                <p v-if="availabilityMessage" class="text-sm mb-4 text-center py-2 px-4 rounded-lg" :class="availabilityMessage.includes('available') && !availabilityMessage.includes('not') ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'">
                                    {{ availabilityMessage }}
                                </p>
